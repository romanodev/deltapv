...
def jaxpv_pn_hetero(G=None):
    ...
    CdS = {'Nc': 2.2e18, 'Nv':1.8e19, 'Eg':2.4, 'eps':10, 'Et': 0,
            'mn':100, 'mp':25, 'tn':1e-8, 'tp':1e-13,
            'Chi': 4.}
    
    CdTe = {'Nc': 8e17, 'Nv': 1.8e19, 'Eg':1.5, 'eps':9.4, 'Et': 0,
            'mn':320, 'mp':40, 'tn':5e-9, 'tp':5e-9,
            'Chi': 3.9}
    ...
    dope = np.ones_like(x)
    dope[CdS_region] = dope[CdS_region] * 1e17
    dope[CdTe_region] = dope[CdTe_region] * (- 1e15)
    simu.doping_profile(dope, range(len(x)))
