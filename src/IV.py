...
def grad_IV( dgrid , Vincrement , eps , Chi , Eg , Nc , Nv , Ndop , mn , mp , Et , tn , tp , Br , Cn , Cp , Snl , Spl , Snr , Spr , G_used ):
    ...
    while cond and ( iter < max_iter ):
        ...
        jac_phis['Chi'] = gradsol[6] + np.dot( gradsol[0] , np.reshape( dneq0_dChi , ( N , 1 ) ) ) + np.dot( gradsol[1] , np.reshape( dneqL_dChi , ( N , 1 ) ) ) + np.dot( gradsol[2] , np.reshape( dpeq0_dChi , ( N , 1 ) ) ) + np.dot( gradsol[3] , np.reshape( dpeqL_dChi , ( N , 1 ) ) ) + np.dot( gradsol[4] , jac_phis['Chi'] )
        jac_phis['Eg'] = gradsol[7] + np.dot( gradsol[0] , np.reshape( dneq0_dEg , ( N , 1 ) ) ) + np.dot( gradsol[1] , np.reshape( dneqL_dEg , ( N , 1 ) ) ) + np.dot( gradsol[2] , np.reshape( dpeq0_dEg , ( N , 1 ) ) ) + np.dot( gradsol[3] , np.reshape( dpeqL_dEg , ( N , 1 ) ) )+ np.dot( gradsol[4] , jac_phis['Eg'] )
        jac_phis['Nc'] = gradsol[8] + np.dot( gradsol[0] , np.reshape( dneq0_dNc , ( N , 1 ) ) ) + np.dot( gradsol[1] , np.reshape( dneqL_dNc , ( N , 1 ) ) ) + np.dot( gradsol[2] , np.reshape( dpeq0_dNc , ( N , 1 ) ) ) + np.dot( gradsol[3] , np.reshape( dpeqL_dNc , ( N , 1 ) ) )+ np.dot( gradsol[4] , jac_phis['Nc'] )
        jac_phis['Nv'] = gradsol[9] + np.dot( gradsol[0] , np.reshape( dneq0_dNv , ( N , 1 ) ) ) + np.dot( gradsol[1] , np.reshape( dneqL_dNv , ( N , 1 ) ) ) + np.dot( gradsol[2] , np.reshape( dpeq0_dNv , ( N , 1 ) ) ) + np.dot( gradsol[3] , np.reshape( dpeqL_dNv , ( N , 1 ) ) )+ np.dot( gradsol[4] , jac_phis['Nv'] )
        jac_phis['Ndop'] = gradsol[10] + np.dot( gradsol[0] , np.reshape( dneq0_dNdop , ( N , 1 ) ) ) + np.dot( gradsol[1] , np.reshape( dneqL_dNdop , ( N , 1 ) ) ) + np.dot( gradsol[2] , np.reshape( dpeq0_dNdop , ( N , 1 ) ) ) + np.dot( gradsol[3] , np.reshape( dpeqL_dNdop , ( N , 1 ) ) )+ np.dot( gradsol[4] , jac_phis['Ndop'] )
