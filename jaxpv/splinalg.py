...
def spget(i, j, values, indices, indptr):
    ...
    try:
        loc = indptr[i] + np.argwhere(indices[indptr[i]:indptr[i + 1]] == j)[0][0]
    ...
    for i in range(1, n):
        ...
        for idx_in_row, k in enumerate(indices[loc_start:loc_end]):
            ...
            if loc_kk is not None:
                fac = ops.index_update(fac, ops.index[loc_ik], fac[loc_ik] / val_kk)
                for idx_in_row, j in enumerate(indices[loc_start:loc_end]):
                    loc_ij = indptr[i] + idx_in_row
                    if j <= k:
                        continue
                    _, val_kj = spget(k, j, fac, indices, indptr)
                    fac = ops.index_update(fac, ops.index[loc_ij], fac[loc_ij] - fac[loc_ik] * val_kj)
...
def spfsub(values, indices, indptr, b):
    ...
    for i in range(1, n):
        ...
        x = ops.index_update(x, ops.index[i],
                             b[i] - np.dot(values[indptr[i]:end],
                                           x[(indices[indptr[i]:end],)]))
...
def spbsub(values, indices, indptr, b):
    ...
    for i in range(n - 2, -1, -1):
        ...
        x = ops.index_update(x, ops.index[i], 
                             (b[i] - np.dot(values[start:indptr[i + 1]],
                              x[(indices[start:indptr[i + 1]],)])) / spget(
                                  i, i, values, indices, indptr)[1])
...
def dense(values, indices, indptr):
    ...
    for i in range(n):
        matrix = ops.index_update(matrix,
                                  ops.index[i, indices[indptr[i]:indptr[i + 1]]],
